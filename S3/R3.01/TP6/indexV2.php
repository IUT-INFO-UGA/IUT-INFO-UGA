...
$change = new ChangeV2('data/devise.csv', 'data/exchangeRate.csv');
...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['montant'], $_GET['source'], $_GET['cible'])) {
    ...
    try {
        ...
        $result = "Le change de $montant {$change->getDevises()[$source]->getName()} en {$change->getDevises()[$cible]->getName()} fait " . number_format($convertedAmount, 2);
    } catch (Exception $e) {
        $result = $e->getMessage();
    }
}
	...
	<main class="w3-container">
		...
		<div>
			<form id="convertisseur" action="indexV2.php" method="get" class="w3-panel w3-card-2">
				<input id="montant" type="number" name="montant" placeholder="Montant" value="<?= isset($_GET['montant']) ? htmlspecialchars($_GET['montant']) : '' ?>" required>
				<select id="source" name="source">
					<?php foreach ($change->getDevises() as $devise): ?>
					<option value="<?= $devise->getIdf() ?>" <?= (isset($_GET['source']) && $_GET['source'] === $devise->getIdf()) ? 'selected' : '' ?>>
						<?= $devise->getName() ?></option>
					<?php endforeach; ?>
				</select>
				<img src="design/arrow-icon.png" alt="">
				<select id="cible" name="cible">
					<?php foreach ($change->getDevises() as $devise): ?>
					<option value="<?= $devise->getIdf() ?>" <?= (isset($_GET['cible']) && $_GET['cible'] === $devise->getIdf()) ? 'selected' : '' ?>>
						<?= $devise->getName() ?></option>
					<?php endforeach; ?>
				</select>
