...
void Mondial::printIslandsInSameArchipelago(const string& islandName) const {
    //cette fonction marche pas je sait pas pourquoi
    ...
    while (otherIsland != nullptr)
    {
        ...
        //c'est ici ou ca marche pas par ce que ma comparaison est nul
        if(Marchipel!=nullptr && (Marchipel->GetText() == archipel->GetText()))
        {
