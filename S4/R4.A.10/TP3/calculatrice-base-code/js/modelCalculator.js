...
export class Calculator {
	...
	setValueEditableButton(idBtn, val) {
		console.log(idBtn, val);
		this._editableButtons[idBtn] = new EditableButton(idBtn, val);
		this.saveStateToClient();
	}

	saveStateToClient() {
		console.log(this._editableButtons);
		window.localStorage.setItem('memoryContent', this._memory);
		window.localStorage.setItem('editableButtonsContent', JSON.stringify(this._editableButtons));
	}

	retrieveStateFromClient() {
		this._memory = window.localStorage.getItem('memoryContent');
		const editableButtonsContent = window.localStorage.getItem('editableButtonsContent');
		if (editableButtonsContent) {
			const buttons = JSON.parse(editableButtonsContent);

			Object.keys(buttons).forEach(key => {
				this._editableButtons[key] = new EditableButton(key, buttons[key]._value);
			});
		}
	}
}
